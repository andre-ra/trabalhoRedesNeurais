...
def treinamentoPMC2(x, y , numNeuronios, n = 0.01, epsilon = 10**-7, th = 0):
    ...
    w2 = 0.1*np.random.rand(numNeuronios[2], w1.shape[0])
    b2 = 0.1*np.random.rand(numNeuronios[2], 1)
    ...
    while (abs((eqm - eqmAnt)) > epsilon):
        ...
        for i in range(0,m):
            ...
            delta2 = e*(1 - np.power(u2, 2))
            dW2 += np.clip(n*delta2*(a1.T), -clip, clip)
            dB2 += np.clip(n*delta2*-1, -clip, clip)                


            delta1 = np.sum(delta2*w2)*(1-np.power(u1, 2))                
    ...
    return w2, b2, w1, b1, w0, b0, eqmLista

def validacaoPMC2(xVal, yVal, w2, b2, w1, b1, w0, b0):

    # Inicializa as variáveis
    m = xVal.shape[0]
    yEst = np.zeros((m, w2.shape[0])) 
